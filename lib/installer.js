...
const { pathExists, isDirectory, copyDirRecursive, copyFileManaged, ensureDir, hashFile } = require("./fs-utils");

function generateCommandsHelp(pluginName, commandsHelp) {
  const lines = [`# ${pluginName} Commands\n`];
  for (const [name, cmd] of Object.entries(commandsHelp)) {
    lines.push(`## ${name}`);
...
function installForCli(options, plugin, cli) {
  ...
  // commands_help: generate <plugin-name>-commands.md from plugin.json declaration
  if (plugin.meta.commands_help && typeof plugin.meta.commands_help === "object") {
    const cmdFileName = `${options.pluginName}-commands.md`;
    const cmdTargetPath = path.join(rootPath, "rules", cmdFileName);
    const cmdContent = generateCommandsHelp(options.pluginName, plugin.meta.commands_help);

    if (!options.dryRun) {
      ensureDir(path.join(rootPath, "rules"));
      fs.writeFileSync(cmdTargetPath, cmdContent, "utf8");
      managedPaths.push({ path: cmdTargetPath, type: "file", hash: hashFile(cmdTargetPath) });
    } else {
      managedPaths.push({ path: cmdTargetPath, type: "file" });
    }
  }
